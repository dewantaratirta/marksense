<script lang="ts">
  ...
  >
    {#each { length: numCircles } as _, i}
      <div
        class="absolute animate-ripple rounded-full bg-primary-300 shadow-xl top-1/2 left-1/2 translate-x-1/2 translate-y-1/2 [--i:{i}]"
        style="width: {mainCircleSize + i * 70}px;
          ...
          : 'solid'};
           border-width: 1px;
           border-color: rgba(var(--color-primary-300), {(5 + i * 5) / 100}) !important;
          "
      ></div>
    {/each}
  </div>
